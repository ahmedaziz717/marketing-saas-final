...
const actionClass = "h-auto min-w-0 w-full whitespace-normal break-words py-3 leading-5";
export function AssetWorkflowActions(props: AssetWorkflowActionsProps) {
  ...
  return <section aria-label={surface === "studio" ? "Studio handoff" : "Library review"} className="min-w-0 space-y-4">
    {surface === "studio" ? <div className="min-w-0 rounded-xl border bg-muted/40 p-4">
      ...
      {working && canCreate && <div className="mt-4 grid min-w-0 gap-3">
        <Button type="button" className={actionClass} disabled={busy} onClick={props.onSubmit}><Send className="mr-2 h-4 w-4 shrink-0" />{state === "draft" ? "Submit for approval" : "Resubmit for approval"}</Button>
        {mayApproveAndAdd(role) && <><Button type="button" className={actionClass} variant="outline" disabled={busy} onClick={props.onApproveAndAdd}><Check className="mr-2 h-4 w-4 shrink-0" />Approve & add to library</Button><p className="text-xs leading-5 text-muted-foreground">You have approval permission. This shortcut records your approval; it does not publish anything.</p></>}
      </div>}
      {!working && <Button className={actionClass + " mt-4"} variant="outline" onClick={props.onViewSubmission}>View {state === "needs_review" ? "submission" : "in library"}</Button>}
    </div> : <div className="min-w-0 rounded-xl border bg-muted/40 p-4">
      ...
      {canDecide && <Button className={actionClass + " mt-4"} disabled={busy} onClick={() => props.onReview("approved")}><Check className="mr-2 h-4 w-4" />Approve</Button>}
    </div>}
    ...
    {canReturn && <div className="grid min-w-0 gap-2 sm:grid-cols-2"><Button className={actionClass} variant="outline" disabled={busy || !note.trim()} onClick={() => props.onReview("changes_requested")}>Request changes</Button><Button className={actionClass} variant="outline" disabled={busy} onClick={() => props.onReview("rejected")}>Reject</Button><p className="text-xs leading-5 text-muted-foreground sm:col-span-2">Explain what needs changing before requesting a revision.</p></div>}
    <Button variant="ghost" size="sm" disabled={busy || !note.trim()} onClick={props.onComment}>Add comment</Button>
    {canCreate && <div className="border-t pt-4"><Button className={actionClass} variant="outline" disabled={busy} onClick={props.onRevise}>{surface === "studio" ? "Upload a revised version" : "Create revised version in Studio"}</Button><p className="mt-2 text-xs leading-5 text-muted-foreground">Revisions are separate drafts. This version and its review history are preserved.</p></div>}
  </section>;
}
