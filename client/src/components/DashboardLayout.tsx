...
import { BarChart3, Megaphone, Users, BookOpenText, Boxes, FolderOpen, Images, LayoutDashboard, LogOut, PackageSearch, PanelLeft, Send, Settings } from "lucide-react";
...
const menuItems = [
  ...
  { icon: Users, label: "Social Media", path: "/app/social" },
  { icon: Megaphone, label: "Advertising", path: "/app/advertising" },
...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  const activeMenuItem = menuItems.find(item => (item.path === location || (item.path !== "/app" && location.startsWith(item.path + "/"))));
