...
export function WebsiteImportWizard({ organizationId, onComplete, scanMode = "brand_and_products" }: { organizationId: number; onComplete: () => void; scanMode?: "brand_and_products" | "products_only" }) {
  ...
  const startImport = async () => {
    try { setRunning(true); setRunStatus("Discovering the public product catalog"); const result = await start.mutateAsync({ organizationId, websiteUrl, maxPages: Number(maxPages), scanMode }); setLiveProgress({ processed: 0, discovered: result.pagesDiscovered }); await run(result.jobId); }
