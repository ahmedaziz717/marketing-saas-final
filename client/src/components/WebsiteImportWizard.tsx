...
export function WebsiteImportWizard({ organizationId, onComplete, scanMode = "brand_and_products" }: { organizationId: number; onComplete: () => void; scanMode?: "brand_and_products" | "products_only" }) {
  ...
  useEffect(() => {
    if (scanMode === "products_only" && job?.sourceUrl && !websiteUrl) setWebsiteUrl(job.sourceUrl);
  ...
  const reportScanError = async (error: unknown) => {
    const message = error instanceof Error ? error.message : "Website import was interrupted.";
  ...
  const run = async (jobId: number, initialStatus = "crawling") => {
    setScanError(null); setRunning(true); setActiveJobId(jobId); let status = initialStatus;
    try {
      while (["queued", "discovering", "crawling"].includes(status)) {
        setRunStatus("Mapping and reading public pages");
        const result = await crawlBatch.mutateAsync({ organizationId, jobId, batchSize: 1 });
      ...
      while (status === "analyzing") {
        setRunStatus(scanMode === "products_only" ? "AI is organizing verified product evidence" : "AI is organizing brand and product evidence");
        const result = await analyzeBatch.mutateAsync({ organizationId, jobId, batchSize: 5 }); status = result.status; if (result.done) break;
      }
      await refresh();
      if (scanMode === "products_only") { toast.success("Product catalog rescan completed and deduplicated"); onComplete(); }
      else toast.success("Website analysis is ready to review");
    } catch (error) { await reportScanError(error); }
    finally { setRunning(false); setActiveJobId(null); }
  };
  const startImport = async () => {
    setScanError(null);
    const pageLimit = Number(maxPages);
    if (!Number.isInteger(pageLimit) || pageLimit < 10 || pageLimit > 250) { toast.error("Choose 10–250 pages for this pass. Use Scan more products again after completion to continue through the next catalog window."); return; }
    try { setRunning(true); setRunStatus("Discovering the next public product catalog window"); const result = await start.mutateAsync({ organizationId, websiteUrl, maxPages: pageLimit, scanMode }); setLiveProgress({ processed: 0, discovered: result.pagesDiscovered }); if (scanMode === "products_only" && result.previouslyCovered > 0) toast.info(`Continuing after ${result.previouslyCovered} previously covered product pages`); await run(result.jobId); }
    catch (error) { setRunning(false); const message = error instanceof Error ? error.message : "Website discovery failed"; await reportScanError(new Error(message.includes("too_big") || message.includes("<=250") ? "A scan pass supports up to 250 pages. Choose 250, then run Scan more products again for the next window." : message)); }
  };
  ...
  if (scanMode === "brand_and_products" && reviewReady && imported && !restart) return <div className="space-y-7">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="eyebrow">Review discovered identity</p><h3 className="mt-2 text-2xl font-semibold">Everything remains editable.</h3></div><div className="flex flex-wrap gap-2"><Button variant="outline" className="rounded-full" onClick={() => setRestart(true)}><ScanSearch className="mr-2 h-4 w-4"/>Start corrected scan</Button><div className="hidden items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800 sm:flex"><ShieldCheck className="h-4 w-4"/>Draft only</div></div></div>
  ...
  return <div><div className="flex items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-primary/10 text-primary"><Globe2 className="h-5 w-5"/></div><div><p className="eyebrow">Website intelligence</p><h3 className="mt-2 text-2xl font-semibold">{scanMode === "products_only" ? "Scan more products" : "Import the company and catalog"}</h3><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">{scanMode === "products_only" ? "Each pass continues after product pages covered by earlier completed scans. Matching canonical URLs or verified SKUs update existing records; genuinely new products are added. Your active brand kit is not changed." : "Frame follows the public sitemap, reads same-site pages in safe batches, and organizes editable brand and product evidence with AI."}</p></div></div><div className="mt-7 grid gap-4 sm:grid-cols-[1fr_150px]"><div><Label>Company website</Label><Input className="mt-2 h-12" value={websiteUrl} onChange={event => setWebsiteUrl(event.target.value)} placeholder="https://company.com" disabled={running}/></div><div><Label>Pages this pass</Label><Input className="mt-2 h-12" type="number" min="10" max="250" value={maxPages} onChange={event => setMaxPages(event.target.value)} onBlur={() => { const value = Number(maxPages); if (Number.isFinite(value)) setMaxPages(String(Math.min(250, Math.max(10, Math.round(value))))); }} disabled={running}/><p className="mt-1 text-[10px] text-muted-foreground">10–250; repeat to continue</p></div></div>{scanError && !running && <div role="alert" className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><p>{scanError}</p>{job && ["crawling", "analyzing", "failed", "cancelled"].includes(job.status) && <p className="mt-2">{job.pagesProcessed} of {job.pagesDiscovered} pages are saved. Choose Resume saved import to continue from that progress.</p>}</div>}{running ? <div className="mt-6 rounded-3xl bg-[#201c25] p-5 text-white"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><Loader2 className="h-4 w-4 animate-spin text-violet-300"/><div><p className="text-sm font-medium">{runStatus}</p><p className="mt-1 text-xs text-white/45">Progress is saved after every request-sized batch.</p></div></div>{activeJobId && <Button size="sm" variant="ghost" className="text-white hover:bg-white/10 hover:text-white" onClick={async () => { await cancel.mutateAsync({ organizationId, jobId: activeJobId }); setRunning(false); setActiveJobId(null); await refresh(); }}><PauseCircle className="mr-2 h-4 w-4"/>Cancel</Button>}</div>{progress && <><Progress className="mt-5 bg-white/10" value={progress.discovered ? Math.min(100, progress.processed / progress.discovered * 100) : 0}/><p className="mt-2 text-right text-[10px] uppercase tracking-wider text-white/45">{progress.processed} of {progress.discovered} pages read</p></>}</div> : <div className="mt-6 flex flex-wrap gap-3"><Button className="h-12 rounded-full px-6" onClick={startImport} disabled={websiteUrl.trim().length < 4 || start.isPending}><ScanSearch className="mr-2 h-4 w-4"/>{scanMode === "products_only" ? "Scan next product pages" : "Analyze full website"}</Button>{job && ["crawling", "analyzing", "failed", "cancelled"].includes(job.status) && <Button variant="outline" className="h-12 rounded-full px-6" onClick={async () => { const result = await resume.mutateAsync({ organizationId, jobId: job.id }); await run(job.id, result.status); }}>Resume saved import</Button>}</div>}<div className="mt-6 flex gap-2 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600"/>{scanMode === "products_only" ? "Deduplication is automatic across every pass: canonical product URL first, verified SKU second. Existing approval states are preserved and source variants are merged." : "Only public HTTP(S) pages on the same company site are read. Private networks, unsafe redirects, unsupported files, and oversized responses are blocked."}</div></div>;
}
