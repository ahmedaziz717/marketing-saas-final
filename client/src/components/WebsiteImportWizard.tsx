...
export function WebsiteImportWizard({ organizationId, onComplete, scanMode = "brand_and_products" }: { organizationId: number; onComplete: () => void; scanMode?: "brand_and_products" | "products_only" }) {
  ...
  const run = async (jobId: number, initialStatus = "crawling") => {
    setRunning(true); setActiveJobId(jobId); let status = initialStatus;
    try {
      ...
      while (status === "analyzing") {
        setRunStatus(scanMode === "products_only" ? "GPT-5.5 is organizing verified product evidence" : "GPT-5.5 is organizing brand and product evidence");
  ...
  const startImport = async () => {
    ...
    catch (error) { setRunning(false); const message = error instanceof Error ? error.message : "Website discovery failed"; toast.error(message.includes("too_big") || message.includes("<=250") ? "A scan pass supports up to 250 pages. Choose 250, then run Scan more products again for the next window." : message); }
  };
