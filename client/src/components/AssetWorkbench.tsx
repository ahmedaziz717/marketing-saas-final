...
import { libraryAssetLink, mayCreateAssets, studioAssetLink, workflowLabel, type AssetSurface } from "@shared/assetWorkflow";
import { AssetWorkflowActions } from "./AssetWorkflowActions";
import { AssetUploadDialog } from "./AssetUploadDialog";
import { AssetExportActions } from "./AssetExportActions";
...
export function AssetWorkbench({ surface }: { surface: AssetSurface }) {
  ...
  const assets = query.data ?? [], selected = assets.find(item => item.key === selectedKey);
  ...
  const inspect = (key: string) => navigate(`${base}${separator}view=${view}&asset=${encodeURIComponent(key)}`);
  ...
  return <>
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">{surface === "studio" ? "Saved work" : "Your shared collection"}</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{surface === "studio" ? "Drafts and experiments stay here. Submit only the versions you want reviewed." : "Only submitted and approved assets appear here. Working drafts stay in Content Studio."}</p></div><div className="flex gap-2"><Button aria-label="Refresh assets" variant="outline" disabled={query.isFetching} onClick={() => query.refetch()}><RefreshCw className={`h-4 w-4 ${query.isFetching ? "animate-spin" : ""}`} /></Button>{canCreate && <Button onClick={() => { setParent(null); setUploadOpen(true); }}><Upload className="mr-2 h-4 w-4" />Upload assets</Button>}</div></div>
