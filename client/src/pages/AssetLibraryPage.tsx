...
import { AssetReviewActions } from "@/components/AssetReviewActions";
...
import { ASSET_STATES, ASSET_VIEWS, assetMatchesView, assetStateLabel, type AssetState, type AssetView, type LibraryAsset } from "@shared/assetLibrary";
...
function Library() {
  ...
  return <>
    ...
    <Dialog open={!!selected} onOpenChange={open => { if (!open) inspect(null); }}>
      <DialogContent className="max-h-[90dvh] overflow-x-hidden overflow-y-auto sm:max-w-5xl">
        <DialogHeader className="min-w-0 pr-8">
          <DialogTitle className="break-words">{selected?.name ?? "Asset"}</DialogTitle>
          <DialogDescription>Review this saved version. Asset approval does not approve a post, caption, schedule, or advertising spend.</DialogDescription>
        </DialogHeader>
        {selected && <div className="grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          <section className="min-w-0">
            <div className="grid min-h-52 place-items-center rounded-2xl bg-muted/50 p-4">
              {selected.mediaType === "video" ? <video key={selected.key} controls preload="metadata" src={selected.url} className="max-h-[50vh] w-full" /> : selected.mediaType === "image" ? <img src={selected.url} alt={selected.name} className="max-h-[50vh] max-w-full object-contain" /> : <Image className="h-12 w-12" />}
            </div>
            {selected.headline && <p className="mt-4 break-words font-semibold">{selected.headline}</p>}
            {selected.primaryText && <p className="mt-2 whitespace-pre-wrap break-words text-sm text-muted-foreground">{selected.primaryText}</p>}
            <a className="mt-4 inline-flex items-center text-sm text-primary" href={selected.url} target="_blank" rel="noreferrer">Open original<ExternalLink className="ml-2 h-4 w-4" /></a>
            {selected.parentKey && <Button variant="link" onClick={() => inspect(selected.parentKey)}>View previous version</Button>}
            <div className="mt-3 flex flex-wrap gap-2">{assets.filter(asset => asset.parentKey === selected.key).map(asset => <Button key={asset.key} className="h-auto max-w-full whitespace-normal break-words py-2" variant="outline" size="sm" onClick={() => inspect(asset.key)}>New version: {asset.name}</Button>)}</div>
          </section>
          <section className="min-w-0 space-y-4">
            <State value={selected.state} />
            <p className="text-sm text-muted-foreground">{selected.purpose === "source" ? "Source material: approval allows use during creation, not direct publishing." : "Finished asset: approval makes this version available for a separate publishing review."}</p>
            {selected.reviewedAtMs && <p className="text-xs text-muted-foreground">Reviewed {new Date(selected.reviewedAtMs).toLocaleString()}</p>}
            <AssetReviewActions
              state={selected.state} canCreate={canCreate} canReview={canReview} busy={busy}
              note={note} onNoteChange={setNote} onReview={decide}
              onSubmit={() => submit.mutate({ organizationId: organizationId!, key: selected.key, revision: selected.revision })}
              onComment={() => comment.mutate({ organizationId: organizationId!, key: selected.key, revision: selected.revision, body: note })}
              onUploadVersion={() => openUpload(selected)}
            />
            <div className="border-t pt-4">
              <h3 className="text-sm font-semibold">Review history</h3>
              {history.isLoading ? <p className="mt-3 text-xs">Loading history...</p> : history.error ? <p role="alert" className="mt-3 text-xs">History could not be loaded.</p> : !history.data?.length ? <p className="mt-3 text-xs text-muted-foreground">No review activity yet.</p> : <div className="mt-3 max-h-56 space-y-3 overflow-y-auto">{history.data.map(item => <div key={item.id} className="rounded-xl bg-muted/50 p-3 text-xs"><p className="font-medium">{assetStateLabel(item.action)} / {item.author}</p>{item.body && <p className="mt-2 whitespace-pre-wrap break-words leading-5">{item.body}</p>}<p className="mt-2 text-muted-foreground">{new Date(item.createdAtMs).toLocaleString()}</p></div>)}</div>}
            </div>
          </section>
        </div>}
      </DialogContent>
    </Dialog>
    <Dialog open={uploadOpen} onOpenChange={open => { if (!uploading) setUploadOpen(open); }}><DialogContent className="max-h-[90dvh] overflow-x-hidden overflow-y-auto"><DialogHeader className="pr-8"><DialogTitle>{parent ? "Upload a new asset version" : "Upload assets"}</DialogTitle><DialogDescription>{parent ? "The original and its approval stay unchanged. This upload becomes a new draft linked to the original." : "Files are saved as drafts. Images: up to 12 MB. MP4/WebM videos: up to 20 MB per file."}</DialogDescription></DialogHeader><div className="space-y-4"><div><Label htmlFor="library-files">{parent ? "New version" : "Choose files"}</Label><Input id="library-files" type="file" className="mt-2" multiple={!parent} accept={types.join(",")} disabled={uploading} onChange={event => setFiles(Array.from(event.target.files ?? []))} /></div><div><Label htmlFor="library-purpose">Asset purpose</Label><select id="library-purpose" className={`${inputClass} mt-2 w-full`} value={purpose} disabled={uploading} onChange={event => setPurpose(event.target.value as "source" | "finished")}><option value="finished">Finished image or video</option><option value="source">Brand / product source material</option></select></div>{purpose === "source" && <div><Label htmlFor="library-source">Source type</Label><select id="library-source" className={`${inputClass} mt-2 w-full`} value={sourceType} disabled={uploading} onChange={event => setSourceType(event.target.value as typeof sourceType)}><option value="logo">Logo</option><option value="product">Product image</option><option value="reference">Reference</option><option value="other">Other</option></select></div>}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={isUgc} disabled={uploading} onChange={event => setIsUgc(event.target.checked)} />User-generated / creator content</label>{isUgc && <label className="flex items-start gap-2 rounded-xl bg-muted p-3 text-sm"><input type="checkbox" className="mt-1" checked={rights} disabled={uploading} onChange={event => setRights(event.target.checked)} />I confirm that we have permission to use this content.</label>}{!!files.length && <p className="text-xs text-muted-foreground">{files.length} file{files.length === 1 ? "" : "s"} selected</p>}{uploadProgress && <p role="status" className="text-sm">{uploadProgress}</p>}<Button className="w-full" disabled={uploading || !files.length || (isUgc && !rights)} onClick={() => void uploadFiles()}>{uploading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save as draft{files.length === 1 ? "" : "s"}</Button></div></DialogContent></Dialog>
  </>;
}
