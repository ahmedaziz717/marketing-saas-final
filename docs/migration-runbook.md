...
Apply `render.yaml` from `codex/render-supabase-migration`, not `main`. Render's initial Blueprint form prompts for the database URL, Supabase publishable and secret keys, owner identity UUID, and AI key. No pre-existing environment group is required. The customer app origin is `https://app.evokeloop.com` and the public website origin is `https://evokeloop.com`; the integration encryption secret is generated automatically. Worker credentials and app origin reference the web service values. Review the paid worker cost before applying the Blueprint.
...
4. Deploy the existing `frame-staging` Render service from this branch with `APP_ORIGIN=https://app.evokeloop.com` and `PUBLIC_SITE_ORIGIN=https://evokeloop.com`. Configure Supabase's Site URL and allowed redirect URLs for `/api/auth/callback` on the app origin. Render remains the host; its generated subdomain is only an incoming legacy address. Follow [Domain cutover](evokeloop-domain-cutover.md) for the complete settings.
