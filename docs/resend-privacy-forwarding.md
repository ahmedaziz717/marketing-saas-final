# Support and privacy email forwarding
...
The signed route is registered after hostname routing, before general JSON parsing and browser CSRF middleware. Signature verification uses Resend's SDK on the unmodified raw body, including timestamp replay protection. Only privacy@evokeloop.com and support@evokeloop.com are accepted (envelope recipients take precedence when provided). Other events and recipients are acknowledged without forwarding. Forwarding back to the receiving domain is disallowed to prevent loops.
...
Both addresses share the existing domain receiving MX record, webhook, credentials, and private forwarding destination. Adding support does not require new DNS or another webhook. The ledger name and idempotency prefix retain their historical privacy names for retry compatibility.
