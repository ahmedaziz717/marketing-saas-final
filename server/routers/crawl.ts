      ...
      for (const candidate of candidates.slice(0, 30)) {
        ...
        if (deterministicRecord) {
          for (const variant of deterministicRecord.variants) {
            ...
            await db.insert(productVariants).values({ organizationId: input.organizationId, productId: product.id, sourceKey, name: variant.name.slice(0, 500), sku: variant.sku?.slice(0, 180) || null, price: variant.price?.slice(0, 80) || null, currency: variant.currency?.slice(0, 16) || null, availability: variant.availability?.slice(0, 120) || null, imageSourceUrl: variant.imageSourceUrl, productUrl: variant.productUrl, metadata: { sourcePageId: candidate.sourcePageId, confidence: "high" }, createdAtMs: now, updatedAtMs: now }).onDuplicateKeyUpdate({ set: { name: variant.name.slice(0, 500), sku: variant.sku?.slice(0, 180) || null, price: variant.price?.slice(0, 80) || null, currency: variant.currency?.slice(0, 16) || null, availability: variant.availability?.slice(0, 120) || null, imageSourceUrl: variant.imageSourceUrl, productUrl: variant.productUrl, metadata: { sourcePageId: candidate.sourcePageId, confidence: "high" }, updatedAtMs: now } });
          }
        }
