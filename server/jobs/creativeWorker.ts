...
export async function processNextBuilderJob(db: Database) {
  ...
  try {
    ...
    await withOrganizationTransaction(db, job.organizationId, async tx => {
      const changed = await tx.update(creativeJobs).set({ status: 'failed', leaseExpiresAtMs: null, completedAtMs: Date.now(), errorMessage: 'Queued generation could not be started. Review the setup and retry.' })
