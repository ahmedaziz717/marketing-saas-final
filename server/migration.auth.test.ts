...
describe('independent authentication boundaries', () => {
  ...
  it('writes Supabase refresh cookies as HttpOnly with correct Express millisecond expiry', () => {
    vi.stubEnv('SUPABASE_URL', 'https://project.supabase.co'); vi.stubEnv('SUPABASE_PUBLISHABLE_KEY', 'test-publishable'); vi.stubEnv('NODE_ENV', 'production');
    ...
    options.cookies.setAll([{ name: 'sb-test', value: 'refreshed', options: { maxAge: 60 } }]);
    expect(res.cookie).toHaveBeenCalledWith('sb-test', 'refreshed', expect.objectContaining({ httpOnly: true, secure: true, sameSite: 'lax', maxAge: 60_000 }));
  });
  it.each(['../other.png', '/absolute.png', 'org-1/../other.png', 'org-1\\other.png'])('rejects unsafe storage paths: %s', key => {
