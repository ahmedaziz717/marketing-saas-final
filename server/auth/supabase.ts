...
export function authClient(req: Request, res: Response) {
  ...
  return createServerClient(url, key, {
    cookies: {
      getAll: () =>
        Object.entries(parse(req.headers.cookie || "")).map(
          ([name, value]) => ({ name, value: value || "" })
        ),
      setAll: cookies => {
        // A request-scoped client can finish its initial session refresh after
        // the response closes. Never mutate a completed response or crash Node.
...
export async function resolveAuthUser(identity: {
  ...
  const imported = await db
    ...
    .from(users)
    .where(sql`lower(${users.email}) = ${identity.email.trim().toLowerCase()}`)
    .limit(1);
  ...
  await db
    ...
    .values({
      ...
      loginMethod: "email",
      role:
        identity.id === process.env.SUPABASE_OWNER_USER_ID ? "admin" : "user",
