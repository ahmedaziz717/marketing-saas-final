...
beforeAll(async () => {
  ...
  const insertedUser = await db.insert(users).values({ openId: `catalog-router-${suffix}`, email: `catalog-router-${suffix}@example.test`, name: "Catalog Router Test", loginMethod: "test" });
  userId = Number(insertedUser[0].insertId);
  const insertedOrg = await db.insert(organizations).values({ name: "Catalog Router Test", slug: `catalog-router-${suffix}`, createdByUserId: userId, createdAtMs: Date.now() });
  ...
  const insertedCreator = await db.insert(users).values({ openId: `catalog-creator-${suffix}`, email: `catalog-creator-${suffix}@example.test`, name: "Catalog Creator Test", loginMethod: "test" });
  creatorUserId = Number(insertedCreator[0].insertId);
  await db.insert(organizationMemberships).values({ organizationId, userId: creatorUserId, role: "creator", status: "active", createdAtMs: Date.now() });
...
describe.sequential("catalog router deletion safeguards", () => {
  ...
  it("removes dependent images, prunes brief selections, and records delete and cleanup events", async () => {
    const db = await getDb(); if (!db) throw new Error("Database unavailable");
    const now = Date.now();
    const insertedJob = await db.insert(websiteCrawlJobs).values({ organizationId, sourceUrl: "https://router-test.example", sourceOrigin: "https://router-test.example", scanMode: "brand_and_products", status: "completed", discoveredUrls: [], cursor: 0, pagesDiscovered: 2, pagesProcessed: 2, maxPages: 10, createdByUserId: userId, createdAtMs: now, updatedAtMs: now, completedAtMs: now });
    const jobId = Number(insertedJob[0].insertId);
    const validPageInsert = await db.insert(websiteCrawlPages).values({ organizationId, jobId, url: "https://router-test.example/products/apex", urlHash: `valid-${suffix}`, pageType: "product", textContent: "Apex", metadata: { productCandidate: true }, colors: [], fonts: [], imageUrls: [], status: "analyzed", fetchedAtMs: now });
    const invalidPageInsert = await db.insert(websiteCrawlPages).values({ organizationId, jobId, url: "https://router-test.example/support/apex", urlHash: `invalid-${suffix}`, pageType: "other", textContent: "Support", metadata: { productCandidate: false }, colors: [], fonts: [], imageUrls: [], status: "analyzed", fetchedAtMs: now });
    const validId = Number((await db.insert(products).values({ organizationId, crawlJobId: jobId, sourcePageId: Number(validPageInsert[0].insertId), name: "Apex", dedupeKey: `valid-product-${suffix}`, productUrl: "https://router-test.example/products/apex", specifications: {}, provenance: {}, status: "pending", createdAtMs: now, updatedAtMs: now }))[0].insertId);
    const invalidId = Number((await db.insert(products).values({ organizationId, crawlJobId: jobId, sourcePageId: Number(invalidPageInsert[0].insertId), name: "Apex Support", dedupeKey: `invalid-product-${suffix}`, productUrl: "https://router-test.example/support/apex", specifications: {}, provenance: {}, status: "pending", createdAtMs: now, updatedAtMs: now }))[0].insertId);
    await db.insert(productImages).values([{ organizationId, productId: validId, sourceUrl: "https://router-test.example/apex.jpg", storageKey: `test/${suffix}/apex.jpg`, url: "https://storage.test/apex.jpg", createdAtMs: now }, { organizationId, productId: invalidId, sourceUrl: "https://router-test.example/support.jpg", storageKey: `test/${suffix}/support.jpg`, url: "https://storage.test/support.jpg", createdAtMs: now }]);
    const briefId = Number((await db.insert(campaignBriefs).values({ organizationId, name: "Router test brief", audience: "Test audience", offer: "Test offer", channel: "meta", placements: ["feed"], formats: ["square_1_1"], creativeDirection: "Test direction", assetIds: [], productIds: [validId, invalidId], status: "approved", createdByUserId: userId, approvedByUserId: userId, approvedAtMs: now, createdAtMs: now, updatedAtMs: now }))[0].insertId);
  ...
  it("rolls back brief pruning and image deletion when the product transaction aborts", async () => {
    const db = await getDb(); if (!db) throw new Error("Database unavailable");
    const now = Date.now();
    const jobId = Number((await db.insert(websiteCrawlJobs).values({ organizationId, sourceUrl: "https://rollback-test.example", sourceOrigin: "https://rollback-test.example", scanMode: "brand_and_products", status: "completed", discoveredUrls: [], cursor: 0, pagesDiscovered: 1, pagesProcessed: 1, maxPages: 10, createdByUserId: userId, createdAtMs: now, updatedAtMs: now, completedAtMs: now }))[0].insertId);
    const pageId = Number((await db.insert(websiteCrawlPages).values({ organizationId, jobId, url: "https://rollback-test.example/products/keep", urlHash: `rollback-${suffix}`, pageType: "product", textContent: "Keep", metadata: { productCandidate: true }, colors: [], fonts: [], imageUrls: [], status: "analyzed", fetchedAtMs: now }))[0].insertId);
    const productId = Number((await db.insert(products).values({ organizationId, crawlJobId: jobId, sourcePageId: pageId, name: "Keep Product", dedupeKey: `rollback-product-${suffix}`, productUrl: "https://rollback-test.example/products/keep", specifications: {}, provenance: {}, status: "pending", createdAtMs: now, updatedAtMs: now }))[0].insertId);
    await db.insert(productImages).values({ organizationId, productId, sourceUrl: "https://rollback-test.example/keep.jpg", storageKey: `test/${suffix}/keep.jpg`, url: "https://storage.test/keep.jpg", createdAtMs: now });
    const briefId = Number((await db.insert(campaignBriefs).values({ organizationId, name: "Rollback brief", audience: "Test audience", offer: "Test offer", channel: "meta", placements: ["feed"], formats: ["square_1_1"], creativeDirection: "Test direction", assetIds: [], productIds: [productId], status: "approved", createdByUserId: userId, approvedByUserId: userId, approvedAtMs: now, createdAtMs: now, updatedAtMs: now }))[0].insertId);
