...
export async function libraryDatabase() {
...
export function normalizeCreative(row: typeof creativeVariants.$inferSelect, workflow?: WorkflowSnapshot): LibraryAsset {
  ...
  return { key: `creative:${row.id}`, name: row.name, origin: "generated", url: row.imageUrl, mediaType: "image", mimeType: "image/*", purpose: "finished", isUgc: false, state: assetState(row.status, workflow, fingerprint), fingerprint, revision: stableHash({ fingerprint, status: row.status, event: workflow?.id ?? null }), parentKey: null, createdAtMs: row.createdAtMs, reviewedAtMs: row.reviewedAtMs, reviewedByUserId: row.reviewedByUserId, headline: row.headline, primaryText: row.primaryText, format: row.format };
}
async function latestWorkflow(db: Runner, organizationId: number, key: string) {
  return (await db.select().from(activityEvents).where(and(eq(activityEvents.organizationId, organizationId), eq(activityEvents.entityType, "library_asset"), eq(activityEvents.entityId, key), inArray(activityEvents.action, WORKFLOW_ACTIONS))).orderBy(desc(activityEvents.id)).limit(1))[0];
}
...
export async function readLibraryAsset(db: Runner, organizationId: number, key: string) {
  let reference;
  try { reference = parseAssetKey(key); } catch { throw new TRPCError({ code: "BAD_REQUEST", message: "Invalid asset reference" }); }
  const workflow = await latestWorkflow(db, organizationId, key);
  if (reference.kind === "asset") {
    const row = (await db.select().from(brandAssets).where(and(eq(brandAssets.organizationId, organizationId), eq(brandAssets.id, reference.id))).limit(1))[0];
    if (row) return normalizeUpload(row, workflow);
  } else {
    const row = (await db.select().from(creativeVariants).where(and(eq(creativeVariants.organizationId, organizationId), eq(creativeVariants.id, reference.id))).limit(1))[0];
    if (row) return normalizeCreative(row, workflow);
  }
