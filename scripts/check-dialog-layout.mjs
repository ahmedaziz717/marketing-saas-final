...
const bundle = await build({
  entryPoints: ["scripts/fixtures/asset-review-dialog.tsx"],
  bundle: true,
  write: false,
  format: "iife",
  platform: "browser",
...
const originalRule =
  '[role="dialog"] button.w-full {position:fixed;left:50%;bottom:1.5rem;z-index:60;width:min(calc(100vw - 3rem),52rem);transform:translateX(-50%)}';
const server = createServer((req, res) => {
  ...
  const reproduce = new URL(req.url, "http://localhost").searchParams.has(
    "reproduce"
  );
  res.end(
...
try {
  ...
  for (const item of cases) {
    ...
    await page("Page.setDocumentContent", {
      ...
      html: `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>${css} *{animation:none!important;transition:none!important;scroll-behavior:auto!important}${item.reproduce ? originalRule : ""}</style></head><body><div id="root"></div></body></html>`,
    });
    await page("Runtime.evaluate", {
      expression: `window.__fixtureQuery=${JSON.stringify(url.search)};` + js,
    });
    let ready = false;
    for (let attempt = 0; attempt < 100; attempt++) {
      ...
      const check = await page("Runtime.evaluate", {
        expression:
          'Boolean(window.runDialogRegression && document.querySelector("[aria-label=\\"Asset review actions\\"]"))',
        returnByValue: true,
      });
    ...
    const evaluation = await page("Runtime.evaluate", {
      expression: "window.runDialogRegression()",
      awaitPromise: true,
      returnByValue: true,
    });
    ...
    await writeFile(
      `artifacts/dialog-layout/${name}.png`,
    ...
    if (item.reproduce) {
      if (report.passed || !report.error.includes("normal flow"))
        throw new Error(
          "The layout test must reproduce the original fixed-button defect"
        );
      console.log(
        "PASS: original overlapping-button defect reproduced by browser regression check"
      );
    } else {
      if (!report.passed) throw new Error(`FAIL ${name}: ${report.error}`);
      console.log(
        `PASS ${name}: no overlap, no horizontal overflow, correct approval visibility, reachable actions`
      );
    }
  }
  await writeFile(
    "artifacts/dialog-layout/results.json",
