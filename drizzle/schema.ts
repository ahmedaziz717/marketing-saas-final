...
export const organizationMembershipsRoleEnum = appSchema.enum("organization_memberships_role", ["owner", "admin", "creator", "reviewer", "publisher"]);
export const organizationMembershipsStatusEnum = appSchema.enum("organization_memberships_status", ["invited", "active", "suspended"]);
export const organizationInvitesRoleEnum = appSchema.enum("organization_invites_role", ["admin", "creator", "reviewer", "publisher"]);
export const organizationInvitesStatusEnum = appSchema.enum("organization_invites_status", ["pending", "accepted", "revoked"]);
export const brandKitsStatusEnum = appSchema.enum("brand_kits_status", ["draft", "active"]);
export const brandAssetsTypeEnum = appSchema.enum("brand_assets_type", ["logo", "font", "product", "reference", "other"]);
export const brandAssetsStatusEnum = appSchema.enum("brand_assets_status", ["pending", "approved", "rejected"]);
export const websiteCrawlJobsScanModeEnum = appSchema.enum("website_crawl_jobs_scanMode", ["brand_and_products", "products_only"]);
export const websiteCrawlJobsStatusEnum = appSchema.enum("website_crawl_jobs_status", ["queued", "discovering", "crawling", "analyzing", "review_ready", "completed", "failed", "cancelled"]);
export const websiteCrawlPagesPageTypeEnum = appSchema.enum("website_crawl_pages_pageType", ["home", "product", "collection", "about", "contact", "other"]);
export const websiteCrawlPagesStatusEnum = appSchema.enum("website_crawl_pages_status", ["fetched", "analyzed", "failed"]);
export const productsRecordTypeEnum = appSchema.enum("products_recordType", ["family", "standalone", "accessory", "material", "software", "service", "bundle"]);
export const productsStatusEnum = appSchema.enum("products_status", ["pending", "approved", "rejected"]);
export const campaignBriefsChannelEnum = appSchema.enum("campaign_briefs_channel", ["meta", "google_display", "microsoft", "multi_channel"]);
export const campaignBriefsStatusEnum = appSchema.enum("campaign_briefs_status", ["draft", "in_review", "approved", "rejected"]);
export const creativeJobsStatusEnum = appSchema.enum("creative_jobs_status", ["queued", "running", "completed", "failed"]);
export const creativeVariantsChannelEnum = appSchema.enum("creative_variants_channel", ["meta", "google_display", "microsoft"]);
export const creativeVariantsStatusEnum = appSchema.enum("creative_variants_status", ["pending", "approved", "rejected"]);
export const reviewCommentsStatusEnum = appSchema.enum("review_comments_status", ["open", "resolved"]);
export const metaConnectionsStatusEnum = appSchema.enum("meta_connections_status", ["disconnected", "connected", "error"]);
export const publishRequestsActionEnum = appSchema.enum("publish_requests_action", ["create", "update"]);
export const publishRequestsStatusEnum = appSchema.enum("publish_requests_status", ["draft", "awaiting_approval", "approved", "publishing", "published", "failed", "cancelled"]);
export const activityEventsOutcomeEnum = appSchema.enum("activity_events_outcome", ["success", "failure"]);
...
export const organizationMemberships = appSchema.table("organization_memberships", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  ...
  status: organizationMembershipsStatusEnum("status").default("active").notNull(),
  createdAtMs: bigint("createdAtMs", { mode: "number" }).notNull(),
}, table => ({ memberIdx: uniqueIndex("organization_member_unique").on(table.organizationId, table.userId) }));

export const organizationInvites = appSchema.table("organization_invites", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  email: varchar("email", { length: 320 }).notNull(),
  role: organizationInvitesRoleEnum("role").notNull(),
  token: varchar("token", { length: 96 }).notNull(),
  status: organizationInvitesStatusEnum("status").default("pending").notNull(),
  invitedByUserId: integer("invitedByUserId").notNull().references(() => users.id),
...
export const brandKits = appSchema.table("brand_kits", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  name: varchar("name", { length: 160 }).notNull(),
  voice: text("voice"),
...
export const brandAssets = appSchema.table("brand_assets", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  brandKitId: integer("brandKitId").notNull().references(() => brandKits.id),
...
export const websiteCrawlJobs = appSchema.table("website_crawl_jobs", {
  ...
  scanMode: websiteCrawlJobsScanModeEnum("scanMode").default("brand_and_products").notNull(),
...
export const websiteCrawlPages = appSchema.table("website_crawl_pages", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  jobId: integer("jobId").notNull().references(() => websiteCrawlJobs.id),
  url: text("url").notNull(),
  urlHash: varchar("urlHash", { length: 64 }).notNull(),
  canonicalUrl: text("canonicalUrl"),
  title: varchar("title", { length: 500 }),
  pageType: websiteCrawlPagesPageTypeEnum("pageType").default("other").notNull(),
...
export const products = appSchema.table("products", {
  ...
  sourcePageId: integer("sourcePageId").references(() => websiteCrawlPages.id),
  name: varchar("name", { length: 300 }).notNull(),
  dedupeKey: varchar("dedupeKey", { length: 64 }).notNull(),
  sku: varchar("sku", { length: 180 }),
  category: varchar("category", { length: 240 }),
  recordType: productsRecordTypeEnum("recordType").default("standalone").notNull(),
...
export const creativeJobs = appSchema.table("creative_jobs", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  briefId: integer("briefId").notNull().references(() => campaignBriefs.id),
  ...
  requestedByUserId: integer("requestedByUserId").notNull().references(() => users.id),
  errorMessage: text("errorMessage"),
...
export const creativeVariants = appSchema.table("creative_variants", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  jobId: integer("jobId").notNull().references(() => creativeJobs.id),
  briefId: integer("briefId").notNull().references(() => campaignBriefs.id),
  name: varchar("name", { length: 180 }).notNull(),
...
export const metaConnections = appSchema.table("meta_connections", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  ...
  status: metaConnectionsStatusEnum("status").default("disconnected").notNull(),
  connectedByUserId: integer("connectedByUserId").notNull().references(() => users.id),
...
export const publishRequests = appSchema.table("publish_requests", {
  id: integer("id").generatedByDefaultAsIdentity().primaryKey(),
  organizationId: integer("organizationId").notNull().references(() => organizations.id),
  variantId: integer("variantId").notNull().references(() => creativeVariants.id),
  connectionId: integer("connectionId").notNull().references(() => metaConnections.id),
